...
#
#   YARN_CONF_DIR  Alternate conf dir. Default is ${HADOOP_HOME/../hadoop-yarn}/conf.
...
if [ "$YARN_LOG_DIR" = "" ]; then
  export YARN_LOG_DIR="$HADOOP_HOME/../hadoop-yarn/logs"
fi
...
case $startStop in
  ...
  (start)
    ...
    if [ "$YARN_MASTER" != "" ]; then
      ...
      rsync -a -e ssh --delete --exclude=.svn --exclude='logs/*' --exclude='contrib/hod/logs/*' $YARN_MASTER/ "$HADOOP_HOME/../hadoop-yarn"
    fi

    hadoop_rotate_log $log
    echo starting $command, logging to $log
    cd "$HADOOP_HOME/../hadoop-yarn"
    nohup nice -n $YARN_NICENESS "$HADOOP_HOME/../hadoop-yarn"/bin/yarn --config $YARN_CONF_DIR $command "$@" > "$log" 2>&1 < /dev/null &
    echo $! > $pid
    sleep 1
    head "$log"
