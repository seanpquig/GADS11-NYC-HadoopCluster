...
HADOOP_CONF_DIR=`readlink -f ${HADOOP_CONF_DIR:-"/etc/hadoop/conf"}`
HADOOP_COMMON_HOME=${HADOOP_HOME}
HADOOP_HDFS_HOME=`readlink -f ${HADOOP_HDFS_HOME:-"$HADOOP_HOME/../hadoop-hdfs"}`
HADOOP_MAPRED_HOME=`readlink -f ${HADOOP_MAPRED_HOME:-"$HADOOP_HOME/../hadoop-mapreduce"}`
HADOOP_YARN_HOME=`readlink -f ${HADOOP_YARN_HOME:-"$HADOOP_HOME/../hadoop-yarn"}`
...
if [ -f "${HADOOP_CONF_DIR}/hadoop-env.sh" -a -z "$HADOOP_ENV_SH" ]; then
  export HADOOP_ENV_SH=1
...
if [ -d "$HADOOP_HOME/../hadoop-hdfs/webapps" ]; then
  CLASSPATH=${CLASSPATH}:$HADOOP_HOME/../hadoop-hdfs/webapps/*
fi
...
if [ -d "${HADOOP_PREFIX}/build/native" -o -d "${HADOOP_PREFIX}/lib/native" ]; then
  ...
  if [ -d "${HADOOP_PREFIX}/lib/native" ]; then
    if [ "x$JAVA_LIBRARY_PATH" != "x" ]; then
      JAVA_LIBRARY_PATH=${JAVA_LIBRARY_PATH}:${HADOOP_COMMON_HOME}/lib/native
    else
      JAVA_LIBRARY_PATH=${HADOOP_COMMON_HOME}/lib/native
    fi
  fi
fi
...
if [ -d "$HADOOP_HDFS_HOME/lib" ]; then
  CLASSPATH=${CLASSPATH}:$HADOOP_HDFS_HOME/lib'/*'
fi

CLASSPATH=${CLASSPATH}:$HADOOP_HDFS_HOME'/*'

# put yarn in classpath if present
if [ -d "$HADOOP_YARN_HOME/lib" ]; then
  CLASSPATH=${CLASSPATH}:$HADOOP_YARN_HOME/lib'/*'
fi
...
if [ "$HADOOP_MAPRED_HOME" != "$HADOOP_YARN_HOME" ] ; then
  CLASSPATH=${CLASSPATH}:${HADOOP_MAPRED_HOME}/'/*':${HADOOP_MAPRED_HOME}/lib/'*'
fi
